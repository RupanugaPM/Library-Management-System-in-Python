                    ...
                    if ans=='y'or ans=='Y':
                            ...
                            if ans2=='y'or ans2=='Y':
                                ...
                                while ans2=='y'or ans2=='Y':
                                    ...
                                    if (a>sl or a<=0)or(a in del_list) :
                                        ...
                                        del_list.append(a)
                                        obj=mc.connect(host="localhost",user="root",passwd="dharso@123",database="mine")
                                        if obj.is_connected():
                                            None
                                    dbcursor=obj.cursor()
                                    dbcursor.execute("delete from book where Sno={}".format(a))
                                    obj.commit()
                        ...
                        while (ans=='y'or ans=='Y') and truth==1:
                            ...
                            if obj.is_connected():
                                None
                            dbcursor=obj.cursor()
                            dbcursor.execute("insert into book values(%s,'%s','%s','%s')"%(sl,aname,bname,gen))
                            obj.commit()
                        ...
                        try:
                            ...
                            if n2==1:
                                while True:
                                    ...
                                    obj=mc.connect(host="localhost",user="root",passwd="dharso@123",database="mine")
                                    if obj.is_connected():
                                        print("The Library is::")
                                    dbcursor=obj.cursor()
                                    dbcursor.execute("select * from book")
                                    data=dbcursor.fetchall()
                                    t=prettytable.PrettyTable(["S.no","Author's Name","Book's Name","Genre"])
                                    l1=[]
                                    for row in data:
                                        for i in row:
                                            l1.append(i)
